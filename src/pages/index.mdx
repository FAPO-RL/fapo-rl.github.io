...
<Header
  ...
  links={[
    ...
    {
      name: "Collection",
      url: "https://huggingface.co/collections/dyyyyyyyy/fapo-68e651650ba668ae5b541f1c",
...
<HighlightedSection>
  ...
  In the context of reinforcement learning, we call these _"confident guesses"_ as _"flawed positives"_, where such flawed guessing rollouts are treated as confident positive signals for policy optimization, thereby reinforcing unreliable reasoning patterns.
...
This design guides the model toward precise error localization and fosters genuine error-detection ability, rather than mere guessing.
...
### Theoretical Analysis about FAPO

The advantage estimation of FAPO can be expressed as that of GRPO:
<LaTeX formula="
\begin{cases}
...
\end{cases}
" />
Our theoretical analysis demonstrate that:
optimization direction moves from warm-up to refinement if the ratio of positive (<LaTeX inline formula="\alpha" />) to negative (<LaTeX inline formula="\beta" />) samples (<LaTeX inline formula="\rho = \frac{\alpha}{\beta}" />) exceeds <LaTeX inline formula="\frac{2}{\lambda} - 1" />.
Furthermore, when <LaTeX inline formula="\rho > \frac{4}{\lambda} - 1" />, the advantage for positive samples is downscaled to stabilize training.
